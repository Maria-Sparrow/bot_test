...
def start(bot, update):
    update.message.reply_text("Utube Music Downloader")


def music(bot, update):
    title, video_url = search_youtube(update.message.text)
    update.message.reply_text("Start downloads " + title)
    music_dict = download(title, video_url)
    update.message.reply_text("Convert tp mp3 " + title)
    update.message.reply_audio(**music_dict, timeout=9999)
    os.remove(title + '.mp3')


def main():
    u = Updater('1469820009:AAGRz9N23mnaUAD8SZRjKGQhcCFpDOkpSWY')
    dp = u.dispatcher
